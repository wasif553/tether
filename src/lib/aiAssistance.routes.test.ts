...
// string AiBrainstormPanel's (now single) starter button sends, imported
...
// Simplify Brainstorm prompt controls — STARTER_ACTIONS was trimmed to
// exactly one entry, so the content-independent rate-limiting tests
// below (which need SEVERAL genuinely DISTINCT prompt strings, never
// about which button they came from) can no longer slice it. These are
// the same literal strings the removed starter buttons used to send —
// preserved here only so those tests keep exercising several distinct,
// realistic prompts; production no longer sends anything but the one
// remaining STARTER_ACTIONS prompt or whatever the student types.
