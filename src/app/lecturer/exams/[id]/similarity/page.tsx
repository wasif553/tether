...
export default function SimilarityReviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return <p className="text-lecturer-text-secondary">Loading similarity review...</p>;
  if (loadError) {
    return (
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <div className="flex items-center justify-between">
        <h1 className="text-[28px] font-bold text-lecturer-text-primary">Answer similarity review</h1>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 rounded border border-lecturer-border p-4 text-sm sm:grid-cols-3">
        <div>
          <p className="text-xs uppercase text-lecturer-text-secondary">Analysis status</p>
          <p className="mt-1">{data?.status ?? "Not yet run"}</p>
        </div>
        <div>
          <p className="text-xs uppercase text-lecturer-text-secondary">Submissions analysed</p>
          <p className="mt-1">{data?.summary?.submissionsAnalysed ?? "—"}</p>
        </div>
        <div>
          <p className="text-xs uppercase text-lecturer-text-secondary">Requiring review</p>
          <p className="mt-1">{needsReviewCount}</p>
        </div>
        <div>
          <p className="text-xs uppercase text-lecturer-text-secondary">High similarity pairs</p>
          <p className="mt-1">{highCount}</p>
        </div>
        <div>
          <p className="text-xs uppercase text-lecturer-text-secondary">Last analysed</p>
          <p className="mt-1">{data?.analysedAt ? new Date(data.analysedAt).toLocaleString() : "Never"}</p>
        </div>
        <div>
          <p className="text-xs uppercase text-lecturer-text-secondary">Algorithm version</p>
      ...
      <div className="mt-3 space-y-4">
        {(!data || data.matches.length === 0) && (
          <p className="text-lecturer-text-secondary">No similarity signals found yet. Run the analysis above.</p>
        )}
        {data?.matches.map((m) => {
          const risk = m.detail?.risk ?? "NONE";
          return (
            <div key={m.id} className="rounded border border-lecturer-border p-4 text-sm">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="rounded bg-lecturer-border-subtle px-2 py-0.5 text-xs text-lecturer-text-secondary">{m.reviewStatusLabel}</span>
              </div>

              {m.question && (
                <p className="mt-2 text-xs text-lecturer-text-secondary">Question {m.question.order + 1}: {m.question.text}</p>
              )}
              ...
              {m.detail?.summary && <p className="mt-2 text-lecturer-text-primary">{m.detail.summary}</p>}
              ...
              {m.detail?.sharedPhraseExcerpt && (
                <p className="mt-2 rounded border border-lecturer-border-subtle bg-lecturer-border-subtle p-2 text-xs italic text-lecturer-text-secondary">
