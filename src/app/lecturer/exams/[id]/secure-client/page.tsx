...
export default function SecureClientConfigurationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <Link href={`/lecturer/exams/${id}`} className="text-sm text-gray-500">
      ...
      <section className="mt-6 rounded border border-gray-200 p-4">
        ...
        <div className="mt-2 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
          ...
          <div>
            <p className="text-xs text-gray-500">Configuration hash</p>
          ...
          <tbody>
            {(sebConfig?.sebKeys ?? []).map((k) => (
              <tr key={k.id} className="border-t border-gray-100">
                ...
                <td>
                  {k.active && (
                    <button onClick={() => revokeKey(k.id)} className="text-xs text-red-600">
                      Revoke
                    </button>
                  )}
                </td>
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
          <select value={newKeyType} onChange={(e) => setNewKeyType(e.target.value as "BROWSER_EXAM_KEY" | "CONFIG_KEY")} className="rounded border border-gray-300 px-2 py-1 text-sm">
          ...
          <tbody>
            {sessions.map((s) => (
              <tr key={s.id} className="border-t border-gray-100">
                <td className="py-1">
                  <Link href={`/lecturer/secure-client/sessions/${s.id}`} className="text-blue-600 hover:underline">
                    {s.studentName}
                  </Link>
                </td>
                <td>{s.clientType}</td>
