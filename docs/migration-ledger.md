...
| 23 | `docs/auth-token-abuse-protection-v1-migration.sql` | Auth and Token Abuse Protection v1 | **APPLIED ONCE — 2026-08-20** | **APPLIED ONCE — 2026-08-20 (same shared database as Preview)** | Written 2026-08-20, on branch `feature/auth-token-abuse-protection-v1`. Independent security review (three review passes — see the feature branch's commit history: initial implementation, then two correctness-fix passes) explicitly authorized applying this migration. Preview and Production share ONE Supabase database (project ref `ugckdvbjzauvcovcqebw`, host `aws-1-ap-northeast-1.pooler.supabase.com`) — applied **once**. Fully additive: ONE new table, `SecurityRateLimitBucket` (7 columns, no foreign keys by design) — no column added to `User`, `Exam`, `PasswordResetToken`, `CourseEnrollmentInvitation`, or any other existing table, no existing row modified. Connected directly with `pg`'s `Client` using the repository's existing `DATABASE_URL` — no `prisma db push`/`migrate`, no Supabase SQL Editor session. Pre-check (read-only) confirmed the table did not yet exist. Applied the file's exact statements (1 `CREATE TABLE`, 2 `CREATE INDEX` — no `ALTER TABLE ADD CONSTRAINT`, since this table has no foreign keys by design) inside a single `BEGIN`/`COMMIT` transaction. Post-apply verification (read-only) confirmed: all 7 columns present with the documented types/nullability/defaults (`keyHash`/`scope` TEXT NOT NULL, `windowStart`/`updatedAt` TIMESTAMP(3) NOT NULL, `count` INTEGER NOT NULL DEFAULT 0, `createdAt` TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP); primary key on `id`; the unique index on (`scope`, `keyHash`) plus the plain index on `windowStart` (3 indexes total including the implicit primary-key index); 0 foreign keys; the new table has 0 rows; `User` column count unchanged (10) and row count unchanged (297); `PasswordResetToken` column count unchanged (6); `CourseEnrollmentInvitation` column count unchanged (10); `Exam` column count unchanged (21). A rollback-only sanity test (insert one synthetic, clearly-non-production row inside a transaction, select it, `ROLLBACK`) confirmed the table is genuinely transactional and left 0 rows behind. Applied AFTER the feature's full focused test suite and three full `npm run release:validate` passes (final: 183/183 test files, 3208/3208 tests, typecheck, lint, build) against the disposable local database — never against this shared database. See docs/auth-token-abuse-protection-v1.md for the feature. **Row 22 (PasswordResetToken) remains already-applied and was NOT touched or re-applied by this row.** |
| 24 | `docs/exam-archive-lifecycle-v1-migration.sql` | Exam Archive Lifecycle v1 (lecturer archive/restore/safe-delete + marks export) | **NOT APPLIED — preview branch only** | **NOT APPLIED — production DB is the same shared database as Preview** | Written 2026-08-24, on branch `preview/ui-redesign-2026-08-24`. **PRODUCTION MIGRATION APPROVAL REQUIRED BEFORE MERGE.** Per this ledger's own topology note, Preview and Production share ONE Supabase database — this task explicitly required NOT executing any schema change against that shared database, so this migration was applied ONLY to a local disposable Postgres container (via `npm run release:validate`'s `prisma db push` step, destroyed immediately after each run) for this feature's own development/test cycle. Fully additive: ONE new nullable column, `Exam.archivedAt TIMESTAMP(3)` — no new table, no new enum, no column added to any other table, no existing row's data touched (a nullable `ADD COLUMN` never populates existing rows). Applied AFTER the feature's full focused test suite (`src/lib/examArchiveLifecycle.routes.test.ts`, 33 cases covering archive/restore filtering, permanent-delete eligibility, and the new marks-export CSV route) and a full `npm run release:validate` pass (210/210 test files, 3945/3945 tests, typecheck, lint, build) against the disposable local database — never against this shared database. Do not apply `docs/exam-archive-lifecycle-v1-migration.sql` to the shared Preview/Production database until an authorized operator has reviewed it and this branch has been approved to merge — see that file's own header for the exact pre-check/post-check verification queries to run first. See docs/exam-archive-lifecycle-v1.md for the feature. |
| 25 | `docs/question-bank-exam-pools-v1-migration.sql` | Question Bank / Exam Pools redesign v1 (reorganised Questions tab, Add-from-Question-Bank flow, required-vs-pool toggling, manual/AI save-to-bank) | **NOT APPLIED — feature branch only** | **NOT APPLIED — production DB is the same shared database as Preview** | Written on branch `feature/question-library-exam-pools`, after explicit user approval of this specific additive migration (the task's own "STOP before changing Prisma, report, wait for approval" gate — approved before any schema edit was made). Per this ledger's own topology note, Preview and Production share ONE Supabase database — this task explicitly required NOT executing any schema change against that shared database, so this migration was applied ONLY to a local disposable Postgres container (via `npm run release:validate`'s `prisma db push` step, destroyed immediately after each run) for this feature's own development/test cycle. Fully additive: TWO new nullable columns on the existing `Question` table (`source TEXT`, `sourceBankQuestionId TEXT`), two new indexes, and one new foreign key (`Question.sourceBankQuestionId` -> `BankQuestion.id`, `ON DELETE SET NULL`) — no new table, no new enum, no column added to any other table, no existing row's data touched (nullable `ADD COLUMN`s never populate existing rows, so every pre-existing `Question` simply reads `source = NULL, sourceBankQuestionId = NULL` — its real provenance is genuinely unrecorded and is never guessed or backfilled). Do not apply `docs/question-bank-exam-pools-v1-migration.sql` to the shared Preview/Production database until an authorized operator has reviewed it and this branch has been approved to merge — see that file's own header for the exact pre-check/post-check verification queries to run first. See docs/question-bank-exam-pools-v1.md for the feature. |
| 26 | `docs/pool-selection-refinement-v1-migration.sql` | Pool Selection Refinement v1 (automatic bank→pool quota selection, per-difficulty student draw, MCQ expanded review, non-mutating Exam Preview) | **NOT APPLIED — feature branch only** | **NOT APPLIED — production DB is the same shared database as Preview** | Written on branch `feature/question-library-exam-pools`, after explicit user approval of this specific additive migration (same "STOP before changing Prisma, report, wait for approval" gate as row 25 — approved before any schema edit was made). Per this ledger's own topology note, Preview and Production share ONE Supabase database — this task explicitly required NOT executing any schema change against that shared database, so this migration was applied ONLY to a local disposable Postgres container (via `npm run release:validate`'s `prisma db push` step, destroyed immediately after each run) for this feature's own development/test cycle. Fully additive: ONE new nullable column on the existing `Question` table (`difficulty TEXT`) and THREE new nullable columns on the existing `QuestionPool` table (`drawCountEasy INTEGER`, `drawCountMedium INTEGER`, `drawCountHard INTEGER`) — no new table, no new enum, no column added to any other table, no existing row's data touched (nullable `ADD COLUMN`s never populate existing rows, so every pre-existing `Question` reads `difficulty = NULL` and every pre-existing `QuestionPool` reads all three quota columns `NULL`, meaning it continues to use its existing single `drawCount` exactly as before — zero behavior change for any pool that predates this feature). Do not apply `docs/pool-selection-refinement-v1-migration.sql` to the shared Preview/Production database until an authorized operator has reviewed it and this branch has been approved to merge — see that file's own header for the exact pre-check/post-check verification queries to run first. See docs/pool-selection-refinement-v1.md for the feature. |
| 27 | `docs/voided-submission-recovery-v1-migration.sql` | VOIDED-attempt recovery v1 (lecturer-facing "Void technical attempt and allow restart" for stale secure-policy submissions) | **APPLIED ONCE — 2026-09-13** | **APPLIED ONCE — 2026-09-13 (same shared database as Preview)** | Written on branch `feature/voided-submission-recovery`, after explicit user approval ("APPROVED FOR PREVIEW"). Preview and Production share ONE Supabase database (project ref `ugckdvbjzauvcovcqebw`) — applied **once**. Fully additive: ONE new `SubmissionStatus` enum value (`VOIDED`) — no new table, no column added to any table, no existing row's `status` changed. Connected directly with `pg`'s `Client` using the repository's existing `DATABASE_URL` — no `prisma db push`/`migrate`, no Supabase SQL Editor session. Pre-check (read-only) confirmed `SubmissionStatus` was `['IN_PROGRESS', 'SUBMITTED', 'GRADED']` (no `VOIDED`), on a database with 193 existing `Submission` rows. Applied the file's single statement (`ALTER TYPE "SubmissionStatus" ADD VALUE IF NOT EXISTS 'VOIDED'`). Post-apply verification (read-only) confirmed: `SubmissionStatus` now `['IN_PROGRESS', 'SUBMITTED', 'GRADED', 'VOIDED']`; `Submission` row count unchanged at 193; 0 rows have `status = 'VOIDED'`. Applied AFTER the feature's full test suite (backend: `src/lib/voidedSubmissionRecovery.routes.test.ts`, `src/lib/assessmentLifecycle.test.ts`, `src/lib/secureClientPolicy.test.ts`; UI: extended `voidedSubmissionRecovery.routes.test.ts` + `page.test.ts` source-scan tests) and two full `npm run release:validate` passes (backend and UI-wiring commits) against the disposable local database — never against this shared database. See docs/voided-submission-recovery-v1.md for the feature. |
