...
| `SUPABASE_URL` (or `NEXT_PUBLIC_SUPABASE_URL` as a fallback) | Supabase project URL | Server-side only — this app's evidence storage adapter never sends it to the browser, regardless of which of the two var names you set it under. Must be exactly `https://PROJECT_REF.supabase.co` — do **not** append `/storage/v1` or `/rest/v1`; `@supabase/supabase-js` appends the correct API path itself. |
