...
| Evidence-retention automation | PARTIAL (was NOT DONE) | A manual, operator-triggered retention runner now exists (`npm run evidence:retention`, age-based on `capturedAt`, default 180-day window, scoped to screen/camera evidence assets). `--execute` requires an explicit `--institution-id` and `--retention-days` — no deployment-wide destructive path — see `docs/tether-evidence-retention-plan.md`. Not wired into any automatic schedule; that remains an institutional policy decision |
