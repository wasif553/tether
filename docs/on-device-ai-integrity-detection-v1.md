...
### Camera startup readiness

**Problem this fixes:** on first exam start, students would occasionally see/log a false `CAMERA_VIEW_BLOCKED` (or, less commonly, `CAMERA_TOO_DARK`/`NO_PERSON_VISIBLE`) even though the camera was working correctly — refreshing and logging back in made it go away. Root cause: many webcams deliver a few transiently black, dark, or artifacted frames while their auto-exposure/auto-focus settle, **even after** the video element's `readyState`/`videoWidth`/`videoHeight` already report it as "playable." The existing readiness check (now `isVideoFrameReady()` in `src/lib/cameraIntegrityDetection.ts`) only ever verified the latter — it was necessary but not sufficient, so the very first few valid-looking frames could still be genuinely blank/dark and get misclassified.

**The fix is a short warm-up grace period, not a stricter readiness check:**

1. `isVideoFrameReady(video)` — unchanged in spirit, tightened slightly (now also checks `videoHeight`, not just `videoWidth`): true once `readyState >= HAVE_CURRENT_DATA (2)` and both dimensions are non-zero. The detection tick still returns early (a harmless no-op) until this is true — model/inference never runs against a video with no frame at all.
2. The **first tick where `isVideoFrameReady()` is true** records a timestamp (`firstReadyFrameAtRef` in the student page). This is the start of the warm-up window, not the moment `getUserMedia()` resolved.
3. `shouldSuppressCameraIntegrityDuringStartup(firstReadyFrameAt, now, gracePeriodMs = CAMERA_STARTUP_GRACE_PERIOD_MS)` (`src/lib/cameraIntegrityDetection.ts`) returns `true` for `CAMERA_STARTUP_GRACE_PERIOD_MS` (3000ms) after that first ready frame. While it returns `true`, the student page forces every one of `CAMERA_VIEW_BLOCKED` / `CAMERA_TOO_DARK` / `NO_PERSON_VISIBLE` / `POSSIBLE_PHONE_VISIBLE` / `POSSIBLE_SECOND_PERSON_VISIBLE`'s decision to `{ conditionMet: false, shouldEmit: false }` for that tick — **no backend `IntegrityEvent` is logged, and the local violation overlay never shows**, regardless of what the raw frame-quality/object-detection result actually was.
4. Detection/inference keeps **running** every tick throughout the warm-up window (the model still warms up, canvas/luminance computation still executes, and the existing consecutive-observation counters in `DetectionCooldownTracker` keep incrementing) — only the *emission* (backend log + local overlay) is suppressed. This means a signal that's still genuinely present once warm-up ends can confirm and emit quickly, rather than needing several more ticks from a cold start.
5. `cameraStartupPhase()` derives a simple, testable phase (`"waiting_for_first_frame" | "warming_up" | "ready" | "timed_out"`) from the same timing state, shown to the student as a calm "Camera is starting. Please keep your face visible." message next to the camera preview and "Starting camera checks..." in the status badge — never the violation overlay.
6. **Timeout, not an indefinite wait:** if no ready frame ever arrives within `CAMERA_READY_TIMEOUT_MS` (15000ms) of the stream starting (`cameraStreamStartedAtRef`, set the moment `getUserMedia()` resolves), the phase becomes `"timed_out"` and the student sees a clear, non-blocking message — the exam is never blocked or hung, and no logout/refresh is required.
7. **Camera recovery:** `firstReadyFrameAtRef`/`cameraStreamStartedAtRef` are reset to `null` on every successful `startCamera()` call (including a restart after the stream was lost/reacquired via "Restore camera") and on `stopCamera()`. A restarted stream gets its own fresh warm-up window — the grace period is never permanently spent after the very first attempt, and, symmetrically, once a stream's warm-up has elapsed, blocked/dark/no-person detection works exactly as it did before this fix (a camera that becomes genuinely covered/dark minutes into the exam is still flagged normally).

**Not affected by this fix:** phone/second-person detection *thresholds*, the object-detection model, the consecutive-frame confirmation rules, evidence-frame capture logic, or the exam watermark — only *when* emission is allowed to start. Evidence-frame upload is suppressed for free during the warm-up window, since it is only ever triggered from a successful `reportIntegrityEvent()` call for `POSSIBLE_PHONE_VISIBLE`/`POSSIBLE_SECOND_PERSON_VISIBLE`, and those are never invoked while `shouldEmit` is forced `false`.

**Dev diagnostics** (`localStorage.sesAiCameraDebug = "true"`, dev builds only — see "Adaptive cadence" above for the existing gate): logs the startup phase, `readyState`/`videoWidth`/`videoHeight`, whether the first frame has been observed, warm-up start/end timestamps, and — whenever a tick's emission is suppressed — the reason (`no-video-dimensions-yet` or `warm-up-period`). Never logs image/frame/pixel data.
