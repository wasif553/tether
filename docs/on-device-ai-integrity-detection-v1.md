...
### Detection-sampling element readiness

**Problem this fixes (v2.1):** even with the full lifecycle above, a false `CAMERA_VIEW_BLOCKED` ("Camera view may be blocked" — the overlay reason text in `src/lib/aiCameraViolationOverlay.ts`) could still appear right around the moment the camera became `READY` and the visible preview was already working. **Root cause:** on-device detection samples from a *second*, hidden `<video>` element (`detectionVideoRef`, distinct from the primary `videoRef`/`examVideoRef` elements the lifecycle above actually warms up), attached to the stream for the first time only once `cameraStatus` becomes `"granted"` — i.e. exactly at `READY`. Attaching an already-stable `MediaStream` to a brand-new `<video>` sink still restarts *that element's own* decode/render pipeline; it can show one or two transiently blocked/dark frames of its own even though the camera hardware and the other video element are already settled. The detection tick's own frame gate at the time was the old weak check (`readyState >= 2` and non-zero dimensions only) — exactly the insufficient check the whole lifecycle rewrite exists to replace, just reintroduced for this second element — so a couple of bad early ticks on `detectionVideoRef` could satisfy the 2-consecutive-tick `CAMERA_VIEW_BLOCKED`/`CAMERA_TOO_DARK` confirmation rule the instant the overall lifecycle (correctly) reported `READY`.

**The fix composes, rather than replaces, the existing gates:** the detection tick now independently applies the exact same strict `isRenderedFrameValid()` check and `REQUIRED_CONSECUTIVE_RENDERED_FRAMES` (3) requirement to `detectionVideoRef` that the primary lifecycle applies to `videoRef`, then a short additional settle warm-up — `DETECTION_SAMPLING_WARMUP_MS` (1000ms, shorter than the primary `CAMERA_WARMUP_MS` since the underlying stream is already live and stable; this only covers the second element's own attach/first-decode lag). `armed` is now `isDetectionArmed(lifecycleState) && detectionVideoWarmedUp` — **both** must hold. This tracking is local to the detection effect's closure (`let` variables, not a component-level ref), so it naturally resets to zero on every fresh attach, including a heartbeat-triggered camera restart — never carrying a previous run's readiness across.

**Not affected by this fix:** phone/second-person detection *thresholds*, the object-detection model, the consecutive-observation confirmation rules for those signals, or the exam watermark — only *when* the camera is considered ready and *when* emission is allowed to start.
