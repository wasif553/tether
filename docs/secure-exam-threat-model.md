...
- **Camera startup lifecycle (v2)** — see
  ...
  lifecycle (v2)"). `cameraStatus === "granted"` now means the camera
  has reached an explicit `READY` state — 3 consecutive genuinely
  rendered frames (not merely `getUserMedia()` resolving) plus a 2.5s
  settle-time warm-up, with a 15s overall startup timeout, a
  teardown-before-retry idempotent restart (up to 2 bounded automatic
  retries, then a manual "Try camera again" button that never requires
  a page reload/logout), and suppression of permission-dialog-triggered
  window-blur/visibility events during every startup phase. Detection
  thresholds and confirmation rules are unchanged — only when the
  camera is considered ready and when emission is allowed to begin;
